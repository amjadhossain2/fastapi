...
CollectionDep = Annotated[object, Depends(get_collection)]

app = FastAPI(lifespan=lifespan)


@app.post("/heroes/", response_model=Hero)
async def create_hero(hero: Hero, collection: CollectionDep):
...
async def read_hero(hero_id: str, collection: CollectionDep):
    try:
...
async def update_hero(hero_id: str, hero: HeroUpdate, collection: CollectionDep):
    try:
        object_id = ObjectId(hero_id)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid hero ID format") from None
...
async def delete_hero(hero_id: str, collection: CollectionDep):
    try:
        object_id = ObjectId(hero_id)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid hero ID format") from None
